...
<? }?>
			...
			<div class="fr">
				<a href="shop_board_list.php?bo_table=<?=$bo_table?>">목록</a>
...
$sql = "select count(wr_id) c from g5_write_".$bo_table." where wr_is_comment=0  and wr_1='".$com_id."'";
$resc = sql_fetch($sql);
$total_count = $resc[c];

if($bo_table == "")
	$bo_table = "notice";
$sql = "select * from g5_write_".$bo_table." where wr_is_comment=0  and wr_1='".$com_id."' order by wr_id desc  limit $start, $rows";
$res = sql_query($sql);
	...
	?>
			<li>
				<a href="shop_board_detail.php?bo_table=<?=$bo_table?>&wr_id=<?=$row[wr_id]?>&page=<?=$page?>&com_id=<?=$com_id?>">
					<!-- div class="d1"><img src="images/temp_c1.jpg" alt=""></div-->
					<dl>
						<dt><?=$com_str?></dt>
						<dd>
