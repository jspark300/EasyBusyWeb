...
else
{
	...
	//$url = "https://openapi.naver.com/v1/map/geocode?encoding=utf-8&coord=latlng&output=xml&query=".$query;
...
{
	$query = urlencode($sido.$gugun.$dong.$bungi);
	//$url = "https://openapi.naver.com/v1/map/geocode?clientId=GnJ4HNArC0Mh_9qWN39m&encoding=utf-8&coord=latlng&output=xml&query=".$query;
	$url = "http://openapi.map.naver.com/api/geocode?key=4656599e5f320c8f90d2a7454368cfea&encoding=utf-8&coord=latlng&output=xml&query=".$query;
//	echo $url;
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, $url);
