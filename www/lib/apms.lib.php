			...
			if($bo[$k]['html_id']) {
				...
				if($bo[$k]['as_menu']) { //서브메뉴
					...
					for ($j=0; $row1=sql_fetch_array($result1); $j++) {
						...
						$sub_subj = ($row1['bo_'.$mobile.'subject']) ? $row1['bo_'.$mobile.'subject'] : $row1['bo_subject'];
						$list[$z]['sub'][$k]['sub'][$j]['name'] = ($row1['as_'.$mobile.'icon']) ? apms_fa($row1['as_'.$mobile.'icon']).' '.$sub_subj : $sub_subj;
