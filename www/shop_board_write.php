...
$comp = sql_fetch($sql);	
	...
	<header class="header2">
		<div class="btn_back"><a href="shop_board_list.php?bo_table=public&com_id=<?=$com_id?>&"></a></div>
		<div class="txt_tit">
			<p>방문후기 - <?=$comp[name]?></p>
		</div>
	</header>
...
<input type="hidden" name="w" value="<?=$w?>" -->
	...
	<input type="hidden" name="wr_1" value="<?=$com_id?>">
<?php
		$option_cnt = 0;
		$option = '';
		$option_hidden = '';
		if ($is_notice || $is_html || $is_secret || $is_mail) {
			$option = '';
			if ($is_notice) {
				$option .= "\n".'<label class="control-label sp-label"  style="float:left;width:100%;"><input type="checkbox" id="notice" name="notice" value="1" '.$notice_checked.' style="width:20%;"> 공지</label>';
				$option_ctn++;
			}
			...
			if ($is_html) {
				if ($is_dhtml_editor) {
					...
					$option .= "\n".'<label class="control-label sp-label"  style="float:left;width:100%;"><input type="checkbox" id="html" name="html" onclick="html_auto_br(this);" value="'.$html_value.'" '.$html_checked.'  style="width:20%;"> HTML</label>';
					$option_ctn++;
				}
			}
			...
			if ($is_secret) {
				if ($is_admin || $is_secret==1) {
					$option .= "\n".'<label class="control-label sp-label" style="float:left;width:100%;"><input type="checkbox" id="secret" name="secret" value="secret" '.$secret_checked.'  style="width:20%;"> 비밀글</label>';
					$option_ctn++;
				} else {
			...
			if ($is_admin) {
				...
				$option .= "\n".'<label class="control-label sp-label"  style="float:left;width:100%;"><input type="checkbox" id="as_type" name="as_type" value="1" '.$main_checked.'  style="width:20%;"> 메인글</label>';
				$option_ctn++;
			}

			if ($is_mail) {
				$option .= "\n".'<label class="control-label sp-label"  style="float:left;width:100%;"><input type="checkbox" id="mail" name="mail" value="mail" '.$recv_email_checked.' style="width:20%;"> 답변메일받기</label>';
				$option_ctn++;
			}
		}
...
<?php if ($is_file) { ?>
	...
	<?php } ?>
		...
		<div class="btn1">
			...
			<a href="/shop_board_list.php?bo_table=<?php echo $bo_table ?>&com_id=<?=$com_id?>" class="btn btn-black btn-sm" role="button">취소</a>
		</div>
	</section>
</form>
<script type="text/javascript">
